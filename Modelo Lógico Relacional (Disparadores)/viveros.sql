...
CREATE OR REPLACE FUNCTION crear_email() RETURNS TRIGGER AS $crear_email$
   BEGIN
      ...
      ELSIF NEW.EMAIL !~ '^[A-Za-z0-9._%-]+@[A-Za-z0-9.-]+[.][A-Za-z]+$' THEN
        RAISE EXCEPTION 'INVALID EMAIL ENTRY, THE EMAIL STRUCTURE MUST BE "example@domain.com"';
      END IF;
...
/* Create trigger with function crear_email() */
...
CREATE OR REPLACE FUNCTION actualizar_stock() RETURNS TRIGGER AS $actualizar_stock$
   BEGIN
      IF NEW.CANTIDAD > 0 THEN
        IF EXISTS (SELECT STOCK FROM PRODUCTO WHERE ((PRODUCTO.COD_PROD = NEW.PRODUCTO_COD_PROD) AND (PRODUCTO.STOCK > NEW.CANTIDAD))) THEN
          UPDATE PRODUCTO SET STOCK = STOCK - NEW.CANTIDAD WHERE COD_PROD=NEW.PRODUCTO_COD_PROD;
        ELSE
          RAISE EXCEPTION 'THE QUANTITY PURCHASED OF A PRODUCT CANNOT BE GREATER THAN ITS STOCK.';
        END IF;
      ELSE
        RAISE EXCEPTION 'THE QUANTITY PURCHASED OF A PRODUCT MUST BE POSITIVE.';
      END IF;
...
/* Create trigger with function actualizar_stock() */
CREATE TRIGGER trigger_actualizar_stock_before_insert BEFORE INSERT ON PRODUCTO_CLIENTE_EMPLEADO
FOR EACH ROW EXECUTE PROCEDURE actualizar_stock();



/* Create function comprobar_vivienda() */
CREATE OR REPLACE FUNCTION comprobar_vivienda() RETURNS TRIGGER AS $comprobar_vivienda$
   BEGIN
      IF EXISTS (SELECT * FROM VIVIENDA WHERE VIVIENDA.CLIENTE_DNI = NEW.CLIENTE_DNI AND VIVIENDA.MUNICIPIO = NEW.MUNICIPIO AND VIVIENDA.DIRECCION != NEW.DIRECCION) THEN
        RAISE EXCEPTION 'ONE PERSON CANNOT LIVE IN TWO DIFFERENT HOUSES WITHIN THE SAME MUNICIPALITY';
      END IF;

      RETURN NEW;
   END;
$comprobar_vivienda$ LANGUAGE plpgsql;

/* Create trigger with function comprobar_vivienda() */
CREATE TRIGGER trigger_comprobar_vivienda_before_insert BEFORE INSERT ON VIVIENDA
...
-- -----------------------------------------------------
START TRANSACTION;
INSERT INTO CLIENTE (DNI,NOMBRE, APELLIDOS, BONIFICACION, TOTAL_MENSUAL, EMAIL) VALUES ('54115666R', 'Daniel', 'Alvarez Medina', 30, 3, 'danielalvarezmedina@gmail.com');
INSERT INTO CLIENTE (DNI,NOMBRE, APELLIDOS, BONIFICACION, TOTAL_MENSUAL, EMAIL) VALUES ('49684321G', 'Acoidan', 'Mesa Hernandez', 22.5, 2, 'acoidanmesahernandez@gmail.com');
INSERT INTO CLIENTE (DNI,NOMBRE, APELLIDOS, BONIFICACION, TOTAL_MENSUAL, EMAIL) VALUES ('43487653N', 'Alberto', 'Mendoza Rodriguez', 15, 1,'albertomendozarodriguez@gmail.com');
INSERT INTO CLIENTE (DNI,NOMBRE, APELLIDOS, BONIFICACION, TOTAL_MENSUAL, EMAIL) VALUES ('51426002Z', 'Sergio', 'Pitti Armas', 12.2, 1, NULL);
INSERT INTO CLIENTE (DNI,NOMBRE, APELLIDOS, BONIFICACION, TOTAL_MENSUAL, EMAIL) VALUES ('45867321I', 'Diego', 'Rodriguez Perez', 10, 1, NULL);
...
-- -----------------------------------------------------
-- Data for table VIVIENDA
-- -----------------------------------------------------
START TRANSACTION;
INSERT INTO VIVIENDA (CLIENTE_DNI, MUNICIPIO, DIRECCION) VALUES ('54115666R', 'La Laguna', 'Carr. Gral. del Nte., 2');
INSERT INTO VIVIENDA (CLIENTE_DNI, MUNICIPIO, DIRECCION) VALUES ('49684321G', 'Santa Cruz', 'C. de Comodoro Rolin, 1');
INSERT INTO VIVIENDA (CLIENTE_DNI, MUNICIPIO, DIRECCION) VALUES ('43487653N', 'La Laguna', 'Calle Heraclio Sanchez, 43');
INSERT INTO VIVIENDA (CLIENTE_DNI, MUNICIPIO, DIRECCION) VALUES ('45867321I', 'La Laguna', 'Campus de Guajara, s/n, 38071');
